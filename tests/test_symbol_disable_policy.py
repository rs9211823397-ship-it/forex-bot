...
def test_default_disabled_symbols_are_high_minimum_risk_metals():
    assert set(DEFAULT_DISABLED_BROKER_SYMBOLS) == {
        "XAUUSD",
        "XAGUSD",
        "XPTUSD",
        "XPDUSD",
...
def test_default_policy_removes_metals_from_analysis_and_execution():
    ...
    grouped = filter_active_symbols(
        active_symbols(include_paper_only=False),
        disabled,
    )
    ...
    assert set(grouped["crypto"]) == {"BTC-USD", "ETH-USD"}
...
def test_override_is_normalized_and_unknown_symbols_fail_closed():
    assert parse_disabled_broker_symbols(" xauusd, btcusd, XAUUSD ") == (
        "BTCUSD",
        "XAUUSD",
    )
