...
def test_demo_quality_v3_balanced_profile_keeps_size_and_disables_count_stoppages():
    ...
    assert 'AAQTS_MIN_ADX = "16"' in launcher
    assert 'AAQTS_SIGNAL_SCORE_THRESHOLD = "45"' in launcher
    assert 'AAQTS_MIN_SIGNAL_CONFIRMATIONS = "3"' in launcher
    assert 'AAQTS_MIN_TRADE_QUALITY = "45"' in launcher
    assert 'AAQTS_MIN_REGIME_CONFIDENCE = "40"' in launcher
...
def test_quality_v3_balanced_research_cohort_is_isolated_across_runtime_tools():
    ...
    cohort = "runtime/ai_chart_analysis_quality_v3_balanced"
...
def test_quality_v3_balanced_does_not_enable_remote_ai_or_disable_news_filter():
    root = Path(__file__).resolve().parents[1]
    launcher = (root / "scripts" / "windows" / "start-demo-engine.ps1").read_text(
        encoding="utf-8"
    )
...
def test_quality_v3_balanced_writes_non_secret_runtime_profile_manifest():
    ...
    assert 'quality_v3_balanced_runtime_profile.json' in launcher
    assert '$runtimeProfile = [ordered]@{' in launcher
    assert 'profile = "quality_v3_balanced"' in launcher
