...
The dynamic TP rule is implemented as a signal-to-signal reversal: when the
next opposite valid alert arrives, the current position is closed at market and
the new opposite position is opened immediately. No fixed broker TP is
submitted because that future price is unknowable at the original entry.
"""
...
def _normalize_timeframe(value: object) -> str:
    ...
    return {"15m": "15", "15min": "15", "15minute": "15", "15minutes": "15"}.get(text, text)
...
class IndicatorOnlyBroker:
    def __init__(self) -> None:
        ...
        self.mt5 = mt5
        self.fixed_lot = _env_float("AAQTS_INDICATOR_FIXED_LOT", float(MT5_FIXED_LOT), minimum=0.01, maximum=100.0)
        self.stop_percent = _env_float("AAQTS_INDICATOR_STOP_PERCENT", 1.0, minimum=0.05, maximum=20.0)
        self.max_spread_stop_ratio = _env_float(
            "AAQTS_INDICATOR_MAX_SPREAD_STOP_RATIO", float(MT5_MAX_SPREAD_STOP_RATIO), minimum=0.01, maximum=1.0
        )
        self.deviation = _env_int("AAQTS_INDICATOR_DEVIATION_POINTS", 20, minimum=0, maximum=500)
    ...
    def close_position(self, position: Any, *, reason: str) -> dict[str, Any]:
        ...
        request = {
            "action": self.mt5.TRADE_ACTION_DEAL, "symbol": position.symbol, "volume": volume,
            "type": self.mt5.ORDER_TYPE_BUY if closing_buy else self.mt5.ORDER_TYPE_SELL,
            "position": int(position.ticket), "price": round(price, int(info.digits)),
            "deviation": self.deviation, "magic": MAGIC, "comment": f"IND24H EXIT {reason}"[:31],
            "type_time": self.mt5.ORDER_TIME_GTC, "type_filling": self._filling(info),
        }
        result = self._send(request)
        return {"ticket": int(position.ticket), "side": self._side(position), "exit_price": float(getattr(result, "price", 0.0) or price), "deal": int(getattr(result, "deal", 0) or 0)}
    ...
    def open_position(self, tv_symbol: str, side: str, *, indicator: str) -> dict[str, Any]:
        ...
        if self._managed_positions(broker_symbol):
        ...
        if ratio > self.max_spread_stop_ratio:
            raise RuntimeError(f"Spread too wide: spread/stop={ratio:.3f} > {self.max_spread_stop_ratio:.3f}")
        ...
        request = {
            "action": self.mt5.TRADE_ACTION_DEAL, "symbol": broker_symbol, "volume": volume,
            "type": order_type, "price": round(entry, int(info.digits)), "sl": round(stop, int(info.digits)),
            "tp": 0.0, "deviation": self.deviation, "magic": MAGIC,
            "comment": f"IND24H {indicator[:12]}"[:31], "type_time": self.mt5.ORDER_TIME_GTC,
            "type_filling": self._filling(info),
        }
        result = self._send(request)
        return {
            "side": side, "symbol": tv_symbol, "broker_symbol": broker_symbol,
            "entry_price": float(getattr(result, "price", 0.0) or entry), "stop_loss": float(request["sl"]),
            "volume": volume, "deal": int(getattr(result, "deal", 0) or 0), "order": int(getattr(result, "order", 0) or 0),
        }
    ...
    def reverse_on_signal(self, alert: Alert) -> dict[str, Any]:
        ...
        closed = self.close_position(opposite[0], reason="NEXT_SIGNAL_TP") if opposite else None
        opened = self.open_position(alert.symbol, alert.side, indicator=alert.indicator)
        return {"action": "REVERSED" if closed else "OPENED", "closed": closed, "opened": opened, "logical_tp_rule": "previous exit = next opposite signal entry event"}
...
class IndicatorOnlyService:
    def __init__(self) -> None:
        ...
        self.allowed_symbols = {_normalize_symbol(item) for item in allowed_raw.split(",") if item.strip()}
        if not self.allowed_symbols:
            raise RuntimeError("At least one indicator-only symbol must be configured")
        self.started_at = _utc_now()
        duration_hours = _env_float("AAQTS_INDICATOR_DURATION_HOURS", 24.0, minimum=0.1, maximum=24.0)
    ...
    def _append(self, event: dict[str, Any]) -> None:
        with self.log_path.open("a", encoding="utf-8") as handle:
            handle.write(json.dumps({"ts": _utc_now().isoformat(), **event}, sort_keys=True, default=str) + "\n")
    ...
    def _write_status(self, **updates: Any) -> None:
        ...
        try:
            ...
            pass
        current.update({
            "mode": MODE_NAME, "timeframe": "15m", "started_utc": self.started_at.isoformat(),
            "expires_utc": self.expires_at.isoformat(), "allowed_symbols": sorted(self.allowed_symbols),
            "indicator_settings": {
                "LuxAlgo - Liquidity Sweeps": {"swings": 5, "options": "Only Wicks", "extend": True, "max_bars": 300},
    ...
    def _validate_payload(self, payload: dict[str, Any]) -> Alert:
        ...
        return Alert(alert_id, indicator, symbol, timeframe, side, _utc_now(), bar_time, dict(payload))
    ...
    def accept(self, payload: dict[str, Any]) -> tuple[int, dict[str, Any]]:
        ...
        self._append({"event": "ALERT_ACCEPTED", "alert_id": alert.alert_id, "indicator": alert.indicator, "symbol": alert.symbol, "side": alert.side, "bar_time": alert.bar_time})
    ...
    def _worker_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                self._append({"event": "EXECUTED_SIGNAL", "alert_id": alert.alert_id, "indicator": alert.indicator, "symbol": alert.symbol, "side": alert.side, "latency_ms": latency_ms, "result": result})
                self._write_status(state="RUNNING", last_execution={"alert_id": alert.alert_id, "side": alert.side, "symbol": alert.symbol, "latency_ms": latency_ms, "result": result})
            except Exception as exc:
                LOGGER.exception("Indicator-only execution failed")
                self._append({"event": "EXECUTION_BLOCKED", "alert_id": alert.alert_id, "indicator": alert.indicator, "symbol": alert.symbol, "side": alert.side, "error": str(exc)})
...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        if self.path.rstrip("/") == "/health":
            self._json(HTTPStatus.OK, {"ok": True, "mode": MODE_NAME, "timeframe": "15m", "expires_utc": self.service.expires_at.isoformat()})
            return
...
def main() -> None:
    ...
    threading.Timer(max(0.0, (service.expires_at - _utc_now()).total_seconds()), request_stop).start()
    LOGGER.info("%s listening on %s:%s; expires %s; symbols=%s", MODE_NAME, host, port, service.expires_at.isoformat(), sorted(service.allowed_symbols))
    try:
