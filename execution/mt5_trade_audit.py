...
class MT5TradeAudit:
    ...
    def record_entry(
        ...
        *,
        ...
        position_id = int(
            getattr(managed_position, "ticket", 0)
            or getattr(result, "position", 0)
            or getattr(result, "order", 0)
            or 0
        )
        broker_symbol = (
            str(getattr(managed_position, "symbol", ""))
            or str(source_symbol)
        )
        ...
        self._append(
            {
                ...
                "DealTicket": int(getattr(result, "deal", 0) or 0),
                "Symbol": broker_symbol,
                "Side": side,
                "Volume": volume,
                "Price": price,
                "StopLoss": float(
                    getattr(managed_position, "initial_stop_loss", 0.0)
                    or risk_plan["stop_loss"]
                ),
                "TakeProfit": float(
                    getattr(managed_position, "take_profit", 0.0)
                    or risk_plan["take_profit"]
                ),
                "PnL": 0.0,
                "ExitReason": "",
                "BrokerReason": "",
                "Magic": self.executor.config.magic,
    ...
    def _history(self, start: datetime, end: datetime) -> list[Any]:
        ...
        if deals is None:
            raise ExecutionError(
                f"MT5 deal history is unavailable: {self.executor.mt5.last_error()}"
            )
        return list(deals)

    def managed_closed_deals(
        self,
        start_time: datetime,
        end_time: datetime,
    ) -> list[ManagedClosedDeal]:
        start = self.executor._as_utc(start_time, "start_time")
        end = self.executor._as_utc(end_time, "end_time")
        if end < start:
            raise ExecutionError("MT5 history end_time cannot precede start_time")

        # Entries may precede the requested P/L window. AAQTS positions are
        # designed to be short-lived, but a 30-day discovery pad also covers
        # restarts and manual closures without trusting exit-deal magic values.
        discovery_start = start - timedelta(days=30)
        deals = self._history(discovery_start, end)
        entry_values = {
            getattr(self.executor.mt5, "DEAL_ENTRY_IN", 0),
            getattr(self.executor.mt5, "DEAL_ENTRY_INOUT", 2),
        }
        exit_values = {
            getattr(self.executor.mt5, "DEAL_ENTRY_OUT", 1),
            getattr(self.executor.mt5, "DEAL_ENTRY_OUT_BY", 3),
        }
        managed_positions = {
        ...
        for deal in deals:
            ...
            if position_id not in managed_positions:
                continue
            if getattr(deal, "entry", None) not in exit_values:
                continue
    ...
    def closed_position_results(
        ...
        return [
            ...
            for item in self.managed_closed_deals(start_time, end_time)
        ]
    ...
    def sync_closed(self, *, lookback_days: int = 30) -> list[ManagedClosedDeal]:
        ...
        for item in results:
            self._append(
                {
                    "Event": "EXIT",
                    "TimeUTC": item.closed_at.isoformat(),
                    "PositionID": item.position_id,
                    "DealTicket": item.deal_ticket,
                    "Symbol": item.symbol,
                    "Side": "",
                    "Volume": item.volume,
                    ...
                    "BrokerReason": (
                        item.broker_reason if item.broker_reason is not None else ""
                    ),
                    "Magic": self.executor.config.magic,
