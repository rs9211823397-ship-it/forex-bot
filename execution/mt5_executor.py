...
class ExecutionConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.order_send_price_retries, bool) or self.order_send_price_retries < 0:
            raise ValueError("order_send_price_retries must be a non-negative integer")
...
class MT5Executor:
    ...
    def positions(self, symbol: Optional[str] = None, managed_only: bool = True) -> list[Any]:
        ...
        if raw is None:
            raise ExecutionError(f"MT5 positions are unavailable: {self.mt5.last_error()}")
        positions = list(raw)
        if managed_only:
    ...
    def remaining_loss_at_stop(self, position: Any) -> float:
        ...
        projected = self.mt5.order_calc_profit(
            order_type, position.symbol, volume, current_price, stop_loss
        )
    ...
    def _validate_tick_and_spread(self, tick: Any, entry: float, stop_loss: float) -> None:
        ...
        if ratio > float(self.config.max_spread_stop_ratio):
            raise ExecutionError(
                ...
                f"({ratio:.3f} > {self.config.max_spread_stop_ratio:.3f})"
            )

    def _validate_margin(self, symbol: str, order_type: int, volume: float, price: float) -> None:
        calculator = getattr(self.mt5, "order_calc_margin", None)
        if not callable(calculator):
            raise ExecutionError("MT5 order_calc_margin is unavailable; refusing unvalidated entry")
        required_margin = calculator(order_type, symbol, float(volume), float(price))
        if required_margin is None or not isfinite(float(required_margin)) or float(required_margin) < 0:
            raise ExecutionError(f"MT5 could not calculate required margin: {self.mt5.last_error()}")
        account = self.mt5.account_info()
        if account is None:
            raise ExecutionError("MT5 account information is unavailable for margin validation")
        free_margin = float(getattr(account, "margin_free", 0.0) or 0.0)
        if not isfinite(free_margin) or free_margin <= 0:
            raise ExecutionError("MT5 free margin is unavailable or non-positive")
        if float(required_margin) > free_margin + 1e-12:
            raise ExecutionError(
                f"Insufficient free margin: requires {float(required_margin):.2f}, available {free_margin:.2f}"
            )

    def _price_retry_codes(self) -> set[int]:
        return {
    ...
    def _send_new_market_order(self, request: dict[str, Any], side: str, info: Any) -> TradeResult:
        success_code = int(getattr(self.mt5, "TRADE_RETCODE_DONE", 10009))
        retries_left = int(self.config.order_send_price_retries)
        while True:
            result = self.mt5.order_send(request)
            trade_result = self._to_result(
                result,
                result is not None and getattr(result, "retcode", None) == success_code,
            )
            if trade_result.success:
                return trade_result
            # A missing response has an unknown broker outcome. Retrying it could
            # duplicate a fill, so fail closed and let the next cycle reconcile.
            if result is None:
                raise ExecutionError("MT5 order_send returned no response; outcome is ambiguous and was not retried")
            if int(getattr(result, "retcode", -1)) not in self._price_retry_codes() or retries_left <= 0:
    ...
    def place_market_order(
        ...
        *,
        ...
        self._validate_margin(symbol, order_type, volume, price)
        request = {
    ...
    def partial_close(self, position_ticket: int, volume: float, comment: str = "AAQTS partial") -> TradeResult:
        ...
        if requested >= current_volume:
            raise ExecutionError("Partial-close volume must be smaller than the open volume")
    ...
    def _close_volume(self, position: Any, volume: float, comment: str) -> TradeResult:
        ...
        if check is None or getattr(check, "retcode", None) != 0:
            detail = getattr(check, "comment", self.mt5.last_error())
            raise ExecutionError(f"MT5 close order_check rejected the request: {detail}")
        result = self.mt5.order_send(request)
        success_code = getattr(self.mt5, "TRADE_RETCODE_DONE", 10009)
        success = result is not None and getattr(result, "retcode", None) == success_code
        trade_result = self._to_result(result, success, position.ticket)
    ...
    def close_all(self) -> list[TradeResult]:
        ...
        for position in positions:
            try:
                ...
                failures.append(f"{getattr(position, 'ticket', '?')}: {exc}")
                logger.exception("Emergency close failed for ticket %s", getattr(position, "ticket", "?"))
        if failures:
    ...
    def _volume_for_risk(
        ...
        *,
        ...
        if projected is None or not isfinite(float(projected)):
            raise ExecutionError(f"MT5 could not calculate entry risk: {self.mt5.last_error()}")
        ...
        if raw_volume + 1e-12 < minimum:
            raise ExecutionError(
                f"Broker minimum volume would exceed approved risk ({minimum_risk:.2f} > {risk_amount:.2f})"
            )
    ...
    def _normalize_volume(volume: float, info: Any) -> float:
        ...
        if not all(isfinite(value) and value > 0 for value in (minimum, maximum, step)):
            raise ExecutionError("Broker volume specification is invalid")
    ...
    def _ensure_connected(self) -> None:
        ...
        if terminal is not None and account is not None:
            return
        logger.error("MT5 session health check failed; attempting one reconnect")
        try:
            self.mt5.shutdown()
        finally:
            self.connected = False
