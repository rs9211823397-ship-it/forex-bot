...
class MarketData:
    ...
    def _align_provider_candles(data, timeframe):
        ...
        if not aligned_mask.all():
            logger.warning("Dropped %s off-grid partial provider candle(s) for %s", int((~aligned_mask).sum()), normalize_timeframe(timeframe))
        return aligned

    @staticmethod
    def _mt5_timeframe(mt5, interval):
        name = {"15m": "TIMEFRAME_M15", "30m": "TIMEFRAME_M30", "1h": "TIMEFRAME_H1", "1d": "TIMEFRAME_D1"}.get(normalize_timeframe(interval))
    ...
    def _download_yahoo(self, symbol, interval):
        if interval == "15m":
            return yf.download(symbol, period="60d", interval=interval, progress=False, auto_adjust=False)
        if interval in ["1h", "30m"]:
            return yf.download(symbol, period="730d", interval=interval, progress=False, auto_adjust=False)
        return yf.download(symbol, start=LOOKBACK_DAYS, progress=False, auto_adjust=False)

    def _download(self, symbol, interval):
        return self._download_mt5(symbol, interval) if self.provider == "MT5" else self._download_yahoo(symbol, interval)

    def _cached_or_raise(self, symbol, timeframe, error=None, as_of=None):
        if not self.allow_cache_fallback:
            detail = f": {error}" if error is not None else ""
            raise HistoricalDataError(f"Fresh market data unavailable for {symbol}; {self.execution_mode} forbids cached fallback{detail}") from error
        try:
            ...
            logger.warning("Using cached market data for %s %s after provider failure", symbol, timeframe)
            return cached
    ...
    def _assert_fresh(self, frame, symbol, timeframe):
        if frame.empty or "close_time" not in frame.columns:
            raise MarketDataFreshnessError(f"No completed candle timestamp available for {symbol}")
        ...
        if age < pd.Timedelta(0) or age > maximum_age:
            raise MarketDataFreshnessError(f"Stale {timeframe} data for {symbol}: last completed candle closed {last_close.isoformat()} ({age} old; max {maximum_age})")
    ...
    def download_data(self, symbol, interval=None, *, as_of=None, use_cache=True):
        ...
        data = data[required].copy(); data.dropna(inplace=True)
        prepared = self.history.prepare(data, timeframe, as_of=as_of or pd.Timestamp.now(tz="UTC"))
        if prepared.empty:
            if use_cache:
                return self._cached_or_raise(symbol, timeframe, as_of=as_of)
